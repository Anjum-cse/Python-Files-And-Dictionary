...
# The third is an optional parameter called d that has a default
# value of {'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5, 'banana': 3, 'grapes': 2, 'watermelon': 7}.
# Write the function checkingIfIn so that when the second parameter is True,
# it checks to see if the first parameter is a key in the third parameter; if it is,
# return True, otherwise return False.
## But if the second paramter is False, then the function should check to see if the
# first parameter is not a key of the third. If it’s not, the function should
# return True in this case, and if it is, it should return False.

def checkingIfIn(required, direction=True, d={'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5, 'banana': 3, 'grapes': 2, 'watermelon': 7} ):
    if direction == True:
        if required in d:
            return  True
        else :
...
## We have provided the function checkingIfIn such that
# if the first input parameter is in the third, dictionary, input parameter,
# then the function returns that value, and otherwise, it returns False. Follow the instructions
